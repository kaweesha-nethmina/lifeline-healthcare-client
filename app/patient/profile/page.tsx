...
import { Calendar, Activity, FileText, User, Save, Upload, X, Edit } from "lucide-react"
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // Fetch patient-specific profile
        const patientProfileResponse: any = await PatientService.getProfile()
        console.log("Patient profile response:", patientProfileResponse)
        
        if (patientProfileResponse) {
          // Handle both response formats
          const patientData = patientProfileResponse.patientData || 
          ...
          if (patientData) {
            ...
            setFormData(prev => ({
              ...prev,
              phone: patientData.phone_number || prev.phone,
              dateOfBirth: patientData.date_of_birth || prev.dateOfBirth,
              gender: patientData.gender || prev.gender,
              address: patientData.address || prev.address,
              emergencyContact: patientData.emergency_contact || prev.emergencyContact,
              insuranceDetails: patientData.insurance_details || prev.insuranceDetails,
              medicalHistory: patientData.medical_history || prev.medicalHistory,
            }))
          }
        }
      } catch (err) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Update patient-specific profile (insurance and medical history)
      const updatePatientData: any = {}
      if (formData.phone !== (profile.phone_number || "")) {
        updatePatientData.phone_number = formData.phone || null
      }
      if (formData.dateOfBirth !== (profile.date_of_birth || "")) {
        updatePatientData.date_of_birth = formData.dateOfBirth || null
      }
      if (formData.gender !== (profile.gender || "")) {
        updatePatientData.gender = formData.gender || null
      }
      if (formData.address !== (profile.address || "")) {
        updatePatientData.address = formData.address || null
      }
      if (formData.emergencyContact !== (profile.emergency_contact || "")) {
        updatePatientData.emergency_contact = formData.emergencyContact || null
      }
      if (formData.insuranceDetails !== (profile.insurance_details || "")) {
        updatePatientData.insurance_details = formData.insuranceDetails || null
      }
      if (formData.medicalHistory !== (profile.medical_history || "")) {
        updatePatientData.medical_history = formData.medicalHistory || null
      }
      
      // Only make the API call if there are changes
      if (Object.keys(updatePatientData).length > 0) {
        const response: any = await PatientService.updateProfile(updatePatientData)
        console.log("Patient profile update response:", response)
        
        if (response) {
          // Handle both response formats
          const patientData = response.patientData || response.data || response
          if (patientData) {
            setProfile(patientData)
          }
        }
